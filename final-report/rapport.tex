  ...
  \begin{titlepage}
    ...
    \vspace{1cm} {\hfill coordinateur \\} {Agathe \textsc{Herrou}
      \hfill Romain \textsc{Labolle} \\}
       ...
       \vspace{2.5cm}
    \begin{abstract}
Ce document présente le rapport final de notre projet TriComp. Ce projet consiste en le développement d'un logiciel, TriComp, qui permettrait à l'utilisateur d'obtenir les instructions à effectuer pour tricotter l'ouvrage de son souhait, à partir de la description globale de celui-ci.

Nous y détaillons tout le travail que nous avons effectué ainsi que les améliorations auxquelles nous avons pensé.
    \end{abstract}
  \end{titlepage}
\makeatother
...
Le nom TriComp de notre logiciel vient de la contraction de ``Compilateur de tricot''. En effet, le but du logiciel est de transformer une description globale d'un tricot en instructions à effectuer par le tricotteur pour le réaliser. Ces instructions sont écrites en langage naturel. Le logiciel se compose de trois parties :
\begin{itemize}
 \item \textbf{Un langage}, TriLang, qui permet à l'utilisateur de décrire globalement son ouvrage (la forme, la taille, les motifs, ...).
 \item \textbf{Un compilateur} qui effectue la transformation de la description en la liste d'instructions.
 \item \textbf{Une interface graphique} qui permet à la fois de visualiser le tricot décrit grâce à Trilang, de faire appel au compilateur et d'afficher la suite des instructions.
\end{itemize}

La figure \ref{fct} montre shématiquement le fonctionnement du logiciel.
...
% TODO : Donner en annexe un exemple d'instructions complètes générées par notre logiciel

Ainsi, les instructions que renvoit notre logiciel sont à la fois précises comme celles d'un manuel, mais 
également claires et faciles à suivre. 
