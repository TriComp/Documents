...
\frame{
   \begin{center}
        ...
        \hspace*{4cm}
        \includegraphics[height=0.2\textheight]{../img/ens.png}
   \end{center}
   \titlepage
}
...
\frame{
	\frametitle{Le compilateur}
		\begin{itemize}
			...
			\begin{itemize}
				\onslide<4-> \item Vérifications
			    \onslide<5-> \item Nombre d'aiguilles nécessaires % TODO est ce que ca sera fait ?
			\end{itemize}
		\end{itemize}
}
...
\frame{
	\frametitle{L'interface graphique}
		\onslide<2-> 3 objectifs :
		\begin{itemize}
			\onslide<3-> \item Simple d'utilisation
			\onslide<4-> \item Outils d'éditions
			\onslide<5-> \item Appel au compilateur
		\end{itemize}
}
...
\frame{
	\frametitle{Difficultés rencontrées}
		\textcolor{red}{Quelques difficultés rencontrées...}
		\begin{itemize}
			  	\onslide<2-> \item \textcolor{red}{Première étape de conception du langage sous-estimée...} \onslide<3-> \textcolor{green}{mais un lanage plus complet dès le début}
				\onslide<3-> \item Mais également la conception de l'interface (surtout le temps nécessaire)
				\onslide<4-> \item Mise en route difficile
				% TODO : Je vois pas trop comment on peut tourner cela.
    	\end{itemize}
		\onslide<5-> \textcolor{green}{Mais aussi quelques bonnes surprises}
			\begin{itemize}
				\onslide<6-> \item L'intégration des différents modules plus simple que prévue
				\onslide<7-> \item D'importantes simplifications par rapport aux modules % i.e moins de niveaux de langages (inutiles, regroupés),...
			\end{itemize}
}
