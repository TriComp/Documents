...
\frame{
	\frametitle{Le compilateur}
		\begin{itemize}
			...
			\onslide<3-> \item Mais aussi la détection d'erreurs, par exemple la présence d'un cycle dans la description du tricot.
			\onslide<4-> \item Pour ce faire, le compilateur construit un graphe de dépendances, qui indique pour chaque pièce, les pièces qu'il faut tricoter au préalable.
			\item Pour mettre en évidence les sections qui se tricotent d'une traite, ce graphe utilise une représentation en bloc de base, de manière similaire aux compilateurs, afin de simplifier l'analyse du code.
		\end{itemize}
}
