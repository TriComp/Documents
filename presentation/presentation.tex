...
\frame{
    \frametitle{Organisation du logiciel}
        \begin{figure}[!h]
    \centering
...
\frame{
    ...
    \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance= 3.5cm,
      ...
      \node[state]     (user)      [draw=none]                                      	   {Utilisateur};
      \node[state]     (descr)     [shape=rectangle] 		  [below of=user]    	   {Description du tricot};      
      \node[state]     (visuel)    [shape=rectangle]              [below left of=descr]    {Rendu visuel du patron};
      \node[state]     (instr)     [shape=rectangle]              [below right of=descr]   {Instructions};


      \path 
	  (user)   edge[single arrow]  node{TriLang} (descr)
	  (descr)  edge[double]  node{GUI}     (visuel)
		   edge[double]  node{Compilateur} (instr)
        ;
    \end{tikzpicture}
    \end{figure}    
...
\frame{
	...
	\begin{tikzpicture}[scale = 0.05]
		...
		\draw[fleche,color=vert] (15,80) -- (30,100);
		\draw[fleche,color=vert] (45,80) -- (30,100);
		\draw[color=vert] (30,105) node {link};
...
\frame{
  ...
  \begin{itemize}
    ...
    \onslide<2-> \item Détection d'erreurs
    \begin{itemize}
    \item présence d'un cycle dans la description du tricot.
    \end{itemize}
    \onslide<3-> \item Construction d'un graphe de dépendances
    \begin{itemize}
    \item Pour chaque pièce, pièces à tricoter au préalable.
    \end{itemize}
    \onslide<4-> \item Sections à tricoter de manière séquentielle : 
    \begin{itemize}
    \item représentation en blocs de base
    \item similaire aux compilateurs
    \item simplifier l'analyse et la traduction du code
    \end{itemize}
  \end{itemize}
}
...
\frame{
  ...
  \begin{itemize}
    ...
    \end{itemize}
    \onslide<3-> \item Côté compilateur :
    \begin{itemize}
      \item gérer les coutures (à effectuer après le tricot)
    \end{itemize}
  \onslide<4-> \item Amélioration de l'interface graphique
  \end{itemize}
}
