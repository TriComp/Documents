...
Le principe du tricot est de former des boucles de fil (appelées \emph{mailles}), et d'à l'aide d'aiguilles de créer de nouvelles boucles 
imbriquées dans les précédentes, formant ainsi un nouveau \emph{rang} ; ainsi, chaque maille est reliées aux mailles qui lui sont adjacentes, 
...
% Est-ce utile de mettre l'adresse du pad?
...
permettant à l'utilisateur de décrire de manière simple et intuitive ce qu'il veut tricoter.}
    ...
    \textit{Production : Intégrer l'afficahge des instructions dans l'interface graphique.}
...
\newpage
\includepdf[landscape]{calendrier-previsionnel-l.pdf} % si on pouvait mettre la page en format paysage ce serait cool
                                         % Concaténer deux PDF ? -> pas vraiment : il faudrait que dans le pdf de sortie (proposal.pdf), la dernière page se voie en format paysage
