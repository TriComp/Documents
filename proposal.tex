...
\begin{abstract}
  ...
  Ce projet sera réalisé dans le cadre d'une UE ``projet intégré'' de l'ENS de lyon. L'équipe est composée de 7 personnes en M1 d'informatique à l'ENS de lyon. Le projet se déroulera entre octobre et mi-décembre.
  
  Ce document vise à présenter notre proposition de projet. Il détaille la motivation derrière ce projet, la manière dont nous comptons atteindre cet objectif (les différents composants du logiciel) ainsi que l'organisation (les différentes tâches, leur répartition au sein de l'équipe, le calendrier prévisionnel).
\end{abstract}
...
\includepdf{calendrier-previsionnel.pdf} % si on pouvait mettre la page en format paysage ce serait cool
                                         % Concaténer deux PDF ? -> pas vraiment : il faudrait que dans le pdf de sortie (proposal.pdf), la dernière page se voit en format paysage
